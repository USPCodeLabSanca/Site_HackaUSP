          ...
          >
            {editions.map((edition, index) => (
              <SwiperSlide key={edition.id}>
                <div className="flex flex-col group relative rounded-xl overflow-hidden bg-slate-900 border border-highlight/30 hover:border-highlight transition-all duration-300 hover:-translate-y-2 h-full z-10 shadow-lg">

                  <div className="relative w-full h-[220px] overflow-hidden transition-transform duration-500 group-hover:scale-110">
                    <Image 
                      src={edition.image} 
                      alt={`Edição ${edition.theme}`} 
                      fill
